...
@test "rename not implemented" {
	run pw mv "${NAME_A}" <<< "${NAME_B}"
	assert_failure
	assert_output "not available for macos_keychain"
}
...
@test "shows fzf preview for single line notes even when locked" {
	...
	local cmd
	cmd="$("${PROJECT_ROOT}/plugins/macos_keychain/fzf_preview" "" "${PW_KEYCHAIN}" <<< "${KEYCHAIN_TEST_PASSWORD}")"
	cmd=${cmd//\{4\}/"\"${NAME_A}\""}
	cmd=${cmd//\{5\}/"\"${ACCOUNT_A}\""}
	cmd=${cmd//\{6\}/"\"${URL_A}\""}
